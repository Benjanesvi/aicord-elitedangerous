// applet.js
// Kael Veyran — SpaceForce Intel Applet (v5.0)
// Live intel (EDSM + INARA) • Archive search (static BGS guide JSON) • Optional markets via relay
// Lore-friendly errors • Always applet-first • Allow stale with timestamps • NLU router • Caching • Admin debug
...
// --- Your GitHub RAW paths (REQUIRED for archive search)
...
// --- Optional: Market relay (EDDN→HTTP). Leave empty to disable.
const MARKET_API_BASE = ""; // e.g., "https://bsc-eddn.yourdomain.com"

// --- Behavior flags
const LORE_ERRORS = true;            // Convert tech errors to in-universe lines
const ALLOW_STALE = true;            // Never block answers for age; append Last update when known
const SHOW_TEACH_SNIPPETS = true;    // Pull one short archive excerpt to “explain the why” alongside live intel
const HTTP_TIMEOUT_MS = 12000;
const MAX_LINES = 16;                // Hard cap for long lists (stations, bodies) to avoid walls of text

// --- Admin controls (debug/diagnostics)
const ADMINS = new Set([
  "Benjanesvi"
]);

// --- Headers
const UA = { "User-Agent": `${APP_NAME}/${APP_VERSION} (+EDSM+INARA)` };
...
function pctFromUnit(n){
  if (typeof n !== "number") return "N/A";
  const v=(n>=0&&n<=1)?n*100:n; return `${v.toFixed(1)}%`;
}
function msToAgo(ms){
...
const RATE = new Map();
function rateOk(who="default"){
  const now = Date.now();
  const arr = (RATE.get(who)||[]).filter(t => now - t < 5000);
  arr.push(now);
  RATE.set(who, arr);
  return arr.length <= 6;
}
...
function formatCommander(ev){
  ...
  const url = d.inaraURL || "";
  const ql = d.commanderSquadron?.inaraURL || "";
  return bullets([
    `**${name}**`,
    `Ranks: ${ranks}`,
    `Squadron: ${squad}`,
    ql && `Squadron page: ${ql}`,
    url && `INARA profile: ${url}`
  ]);
}
...
function formatSystemSnapshot(sysDoc, factionsDoc){
  ...
  const lines = [];
  lines.push(`**${sysDoc.name}** — Allegiance: ${info.allegiance||"?"}, Gov: ${info.government||"?"}, Pop: ${info.population??"?"}`);
...
function _idfScore(qTokens, text, idf){
  const words = _tok(text), tf={};
...
async function loadArchive(){
  ...
  try{
    const [cRes, iRes] = await Promise.all([
      fetchWithTimeout(BGS_CHUNKS_URL, {headers:UA}),
      fetchWithTimeout(BGS_INDEX_URL, {headers:UA})
    ]);
...
async function archiveSearch(query, limit=5){
  ...
  const scored = chunks.map(c=>{
    ...
    return { c, s: _idfScore(terms, c.text, idf) + boost };
  }).sort((a,b)=>b.s-a.s);
  return scored.slice(0, limit).filter(x=>x.s>0.25).map(x=>{
    ...
    return { page: x.c.page, snippet: sn, tags, terms };
  });
}

// ============ OPTIONAL MARKETS (Relay) ============
...
function parseQuery(qRaw){
  ...
  const wantsDebug = /\bdebug|diag|diagnostic\b/.test(q);
  ...
  let sys = null;
  for (const key of Object.keys(SYSTEM_ALIASES)) if (q.includes(key)) sys = SYSTEM_ALIASES[key];
  ...
  let faction = "";
  if (q.includes("black sun crew")) faction = "Black Sun Crew";
  else if (q.includes("oblivion fleet")) faction = "Oblivion Fleet";
  else if (q.includes("space force")) faction = "Space Force";
  else {
    const mf = q.match(/\bfaction\s+([a-z0-9\- ']+)\b/i);
    if (mf) faction = mf[1].trim();
  }
  ...
  const wantsInfluence = /influence|who (controls?|owns?)|percent|share|states|state\b/i.test(q);
...
export async function run(input){
  try{
    ...
    const debug = debugRequested && isAdmin;
    ...
    if (intent === "system_intel"){
      ...
      const display = sysRes.json?.name || sys;
      const blocks = [];

      const header = sysRes.error
        ? `**${display}** — Intel line held. Using known boards.`
        : formatSystemSnapshot(sysRes.json, facRes.error ? null : facRes.json);
      blocks.push(header);

      const facDoc = facRes.error ? null : facRes.json;
      if (facDoc?.factions?.length){
        const bscLine = formatFactionInSystem(display, facDoc, "Black Sun Crew");
      ...
      const actBlock = (traffRes.error || deathRes.error)
        ...
        : formatTrafficDeaths(display, traffRes.json, deathRes.json);
    ...
    if (intent === "system_status"){
      ...
      let out = formatSystemSnapshot(sysDoc || { name: sys }, e2 ? null : facDoc);

      if (SHOW_TEACH_SNIPPETS){
        const picks = await archiveSearch(`what to do states ${sys}`, 1);
        if (picks.length){
          const p = picks[0];
          out += `\n\n**Doctrine**\n${highlight(p.snippet, p.terms)}\n_Source: Squadron archive (static), p.${p.page}_`;
        }
    ...
    if (intent === "faction_status"){
      ...
      if (okStr(sys)){
        ...
        let out = formatFactionInSystem(sys, facDoc || { factions: [] }, faction);

        if (SHOW_TEACH_SNIPPETS){
          const picks = await archiveSearch(`${faction} influence ${sys} election war lockdown`, 1);
          if (picks.length){
            const p = picks[0];
            out += `\n\n**Doctrine**\n${highlight(p.snippet, p.terms)}\n_Source: Squadron archive (static), p.${p.page}_`;
          }
    ...
    if (intent === "station_market"){
      ...
      const sys = canonicalizeSystem(norm(input?.system));
      const station = norm(input?.station);
      if (!okStr(sys) || !okStr(station)) return "Name both the system and station.";
      const { json, error } = await marketFor(sys, station);
      if (error) return loreErrorLine("network");
      if (!json?.commodities?.length) return `No market board visible for **${station}**, **${sys}**.`;
      const top = json.commodities
        .filter(c=>c.sell!=null)
        .sort((a,b)=> (b.sell??0)-(a.sell??0))
        .slice(0, 12)
        .map(c=>`• ${c.name}: sell ${c.sell}cr | buy ${c.buy??"—"}cr | supply ${c.supply??"?"} | demand ${c.demand??"?"}`)
        .join("\n");
    ...
    if (intent === "commodity_search"){
      ...
      const commodity = norm(input?.commodity), near = canonicalizeSystem(norm(input?.near));
      const maxLy = Number(input?.maxLy||30);
      if (!okStr(commodity) || !okStr(near)) return "Name the commodity and the reference system.";
      const { json, error } = await commodityNear(commodity, near, maxLy);
      if (error) return loreErrorLine("network");
    ...
    if (intent === "bgs_archive_search"){
      ...
      if (looksLive && mSys){
        ...
        preface = (!e1 && !e2) ? formatSystemSnapshot(sysDoc, facDoc) + "\n\n"
                               : loreErrorLine("network") + `\n(Attempted live read for **${system}**)` + "\n\n";
      }
      ...
      const lines = picks.map(p=>{
        const tag = p.tags?.length ? ` (${p.tags.join(" | ")})` : "";
        return `• p.${p.page}${tag}\n${highlight(p.snippet, p.terms)}`;
      }).join("\n\n");
